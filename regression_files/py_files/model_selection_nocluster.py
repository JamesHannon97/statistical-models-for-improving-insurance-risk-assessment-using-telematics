...
mod.fit_gmm(n_components=1, 
            covariance_type='full', 
            cluster_variables=mod.continuous_variables + mod.discrete_variables,
            gmm_scaling = scaling,
            gmm_kwargs={'tol':0.001, 'reg_covar':1e-06, 'max_iter':1000, 'n_init':1, 'init_params':'kmeans', 
...
df = pd.DataFrame({"model":[element for element in file_names for _ in range(len(list(glm_families.keys())))], "mavg":mavg, "glm_family":list(glm_families.keys())})
df.to_csv(path_to_csv+"/model_selection_nocluster.csv", index=False) 